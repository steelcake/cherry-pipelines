...
from ..pipeline import EvmConfig

async def create_row_for_protocol_table(
    address: str, name: str, slug: str, network: str, schema_version: str, pipeline_version: str
) -> pl.DataFrame:
...
def get_liquidity_pool_df(pair_created_logs_df: pl.DataFrame, token_df: pl.DataFrame, factory_address: str) -> pl.DataFrame:
    current_time = int(datetime.now(timezone.utc).timestamp())
    liquidity_pool_df = (pair_created_logs_df
        .join(token_df, left_on="token0", right_on="address", how="left")
...
def get_swap_df(
    ...
    token0_swap_df = (swap_logs_df
        ...
        .join(token_df, left_on="token0", right_on="address", how="left", suffix="_token0")
        .join(token_df, left_on="token1", right_on="address", how="left", suffix="_token1")
...
def get_interaction_df(
    ...
    return pl.concat([swap_interaction_df, deposit_interaction_df, withdraw_interaction_df], how="vertical")